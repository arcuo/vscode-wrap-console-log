...
export function activate(context: vscode.ExtensionContext) {
  ...
  var props = JSON.parse(
    fs.readFileSync(
      path.join(currentContext.extensionPath, "package.json"),
      "utf8"
    )
  ...
  Object.keys(props).forEach((prop, i) => {
    ...
    if (pObj) {
      if (pObj.hasOwnProperty("enum")) {
        ...
        if (!valueValid) {
          vscode.workspace
            .getConfiguration("wrap-console-log")
            .update(pName, undefined);
          console.log(
            `Invalid setting value! '${pName}' has been set to default value '${pObj.default}'`
          );
        }
      }
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap",
      (editor, edit) => handle(Wrap.Inline)
    ),
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.string",
      (editor, edit) => handle(Wrap.Inline, false, false, FormatAs.String)
    ),
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.string.up",
      (editor, edit) => handle(Wrap.Up, false, false, FormatAs.String)
    ),
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.string.down",
      (editor, edit) => handle(Wrap.Down, false, false, FormatAs.String)
    ),
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.prefix",
      (editor, edit) => handle(Wrap.Inline, true)
    ),
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.input",
      (editor, edit) => handle(Wrap.Inline, true, true)
    ),
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.up",
    ...
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.up.prefix",
      (editor, edit) => handle(Wrap.Up, true)
    ),
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.up.input",
      (editor, edit) => handle(Wrap.Up, true, true)
    ),
    vscode.commands.registerTextEditorCommand(
    ...
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.down.prefix",
      (editor, edit) => handle(Wrap.Down, true)
    ),
    vscode.commands.registerTextEditorCommand(
      "console.log.wrap.down.input",
      (editor, edit) => handle(Wrap.Down, true, true)
    )
  );
}

function escapeStringChars(str: string) {
  return `${str}`
    .replace(/'/g, "\\'")
    .replace(/"/g, '\\"')
    .replace(/`/g, "\\`");
...
function handle(
  ...
  new Promise((resolve, reject) => {
    ...
    if (ran == undefined) {
      ...
      const languageVarSeparators = getSetting("languageSkipWordSeparator")[
        doc.languageId
      ];
      ...
      while (languageVarSeparators?.includes(wordPrefixChar)) {
        const prefixWordRan = currentEditor.document.getWordRangeAtPosition(
          ran.start.translate(0, -2)
        );
      ...
      if (prefix) {
        if (getSetting("alwaysInputBoxOnPrefix") == true || input) {
          ...
          const prefixString = getSetting("format.wrap.prefixString");
          wrapData.txt = prefixString
            .replace("$func", getSetting("format.wrap.prefixFunctionName"))
            .replace(/[$]var/g, wrapData.item)
            .replace(`('${wrapData.item}:',`, `('${escapeStringChars(wrapData.item)}:',`);
    ...
    .then((wrap: WrapData) => {
      ...
      switch (target) {
            ...
            currentEditor
              .edit(function (e) {
                if (tLineEmpty && onEmptyAction == "Replace empty") {
                  ...
                  e.insert(
                    new vscode.Position(wrap.line, 0),
                  ...
                  if (setCursorLine == "Target line") {
                    e.insert(
                      new vscode.Position(wrap.line - 1, wrap.idx),
                      wrap.ind.concat(wrap.txt)
                    );
                  } else {
                    e.insert(
                      new vscode.Position(wrap.line, wrap.idx),
                      wrap.txt.concat("\n", wrap.ind)
                    );
                  }
                }
        ...
        case Wrap.Down: {
          ...
          wrap.ind =
            vscode.workspace.getConfiguration("wrap-console-log")[
              "autoFormat"
            ] == true
              ? ""
              : wrap.ind;
            ...
            .edit((e) => {
              ...
              if (wrap.lastLine == false && nxtLine.isEmptyOrWhitespace) {
                ...
                if (onEmptyAction == "Insert and push") {
                  ...
                  console.log(
                    `Invalid config setting! Using 'emptyLineAction' default value`
                  );
                  defaultAction();
                }
              } else {
...
function getSetting(setting: string) {
  ...
  return spl.length == 1
    ? vscode.workspace.getConfiguration("wrap-console-log")[setting]
